...
describe('<Alert/>', () => {
  it('should render an alert with a close button by default', () => {
    ...
    assert.equal(wrapper.find('.alert.alert-success').length, 1,
      'alert classes are attached');
    assert.equal(wrapper.find('.alert[role="alert"]').length, 1,
      'alert has correct aria role');
    assert.equal(wrapper.find('.alert[aria-hidden="false"]').length, 1,
      'alert is visible by default');

    // Default close button
    assert.equal(wrapper.find('button.close').length, 1,
      'close button is rendered');
    assert.equal(wrapper.find('button.close[aria-label="close"]').length, 1,
  ...
  it('should render the appropriate contextual color class', () => {
    ...
    assert.equal(wrapper.find('.alert.alert-danger').length, 1,
      'alert contextual class is dependent on color');
  });

  it('should not render a close button if not dismissable', () => {
    const wrapper = shallow(<Alert color='danger' dismissable={false}>Warning!</Alert>);
    assert.equal(wrapper.find('button.close').length, 0,
  ...
  it('should bind passed onDismiss to close button', () => {
    ...
    const wrapper = mount(<Alert color='danger' onDismiss={onDismiss}>Warning!</Alert>);
